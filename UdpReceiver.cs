...
internal class UdpReceiver : ReceiverBase
{
    ...
    protected override void OnStart()
    {
        ...
        for (int i = 0; i < WORKER_THREADS; i++)
        {
            ...
            _listenerThreads[i] = new Thread(ListenerLoop)
                                      {
                                          Name = string.Format("UdpReceiver[{1}].ListenerLoop[{0}]", i, ToString()),
                                          IsBackground = true,
            ...
            _parserThreads[i] = new Thread(ParserLoop)
                                    {
                                        Name = string.Format("UdpReceiver[{1}].ParserLoop[{0}]", i, ToString()),
                                        IsBackground = true
                                    };
    ...
    {
        return string.Format("UdpReceiver:{0}:{1}", IpAddress ?? "*", Port.ToString(CultureInfo.InvariantCulture));
    }
